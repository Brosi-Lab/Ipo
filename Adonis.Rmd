...
We have results! The problem is that I'm a bit confused about by we're getting here. For the second test, testing differences in drought stratifying by years, we obtain a p-value of 1, which is not correct. 

However, if we do not stratify by year...
...
```{r}
adonis(beta ~ Drought, alld, strata = alld$Site, perm = 999)
```
